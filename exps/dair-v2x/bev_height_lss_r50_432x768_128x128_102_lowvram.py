...
H = 1080
W = 1920
final_dim = (448, 768)  # PHẢI chia hết cho 32! Giảm từ (864, 1536) cho 6GB VRAM
...
backbone_conf = {
    ...
    dict(
        type='ResNet',
        depth=50,
        frozen_stages=3,  # ✅ Tối ưu: Freeze 3 stages đầu của ResNet-50 (đã pretrain ImageNet)
        out_indices=[0, 1, 2, 3],
        norm_eval=False,
        init_cfg=dict(type='Pretrained', checkpoint='torchvision://resnet50'),
    ),  # frozen_stages=3: freeze 3 stages đầu, chỉ train stage 4 + BEV head → nhanh ~30%
    'img_neck_conf':
    dict(
        type='SECONDFPN',
...
class BEVHeightLightningModel(LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW(self.model.parameters(),
                                      lr=lr,
                                      weight_decay=1e-7)
        scheduler = MultiStepLR(optimizer, [35, 45])
    ...
    def train_dataloader(self):
        ...
        train_loader = torch.utils.data.DataLoader(
            ...
            num_workers=4,  # Giữ ở 4 để tránh tốn RAM trên laptop
            drop_last=True,
            shuffle=False,
            collate_fn=partial(collate_fn,
                               is_return_depth=False),
            sampler=None,
            pin_memory=True,          # ✅ Tối ưu: CPU→GPU transfer nhanh hơn ~20%
            persistent_workers=True,  # ✅ Tối ưu: giữ workers alive giữa các epoch, giảm overhead
        )
        return train_loader
    ...
    def val_dataloader(self):
        ...
        val_loader = torch.utils.data.DataLoader(
            ...
            num_workers=4,  # Giữ ở 4 để tránh tốn RAM trên laptop
            sampler=None,
            pin_memory=True,          # ✅ Tối ưu: CPU→GPU transfer nhanh hơn
            persistent_workers=True,  # ✅ Tối ưu: giữ workers alive giữa các epoch
        )
        return val_loader
...
def main(args: Namespace) -> None:
    ...
    checkpoint_callback = ModelCheckpoint(dirpath='./outputs/bev_height_lss_r50_432x768_128x128/checkpoints', filename='{epoch}', every_n_epochs=5, save_last=True, save_top_k=-1)
    ...
    if args.evaluate:
        ...
        backup_codebase(os.path.join('./outputs/bev_height_lss_r50_432x768_128x128', 'backup'))
        trainer.fit(model)
...
def run_cli():
    ...
    parser.set_defaults(
        profiler='simple',
        deterministic=False,
        max_epochs=50,          # ✅ Tối ưu: giảm 100 → 50 epochs (tác giả dùng 49 epochs)
        accelerator='ddp',
        num_sanity_val_steps=0,
        gradient_clip_val=5,
        limit_val_batches=0,
        enable_checkpointing=True,
        amp_backend='native',   # ✅ Tối ưu: Mixed Precision Training
        precision=16,           # ✅ Tối ưu: FP16 → nhanh ~1.5x, tiết kiệm ~40% VRAM
        batch_size_per_device=2, # BatchNorm cần batch >= 2. FP16 + frozen ResNet → vừa 6GB
        default_root_dir='./outputs/bev_height_lss_r50_432x768_128x128')
